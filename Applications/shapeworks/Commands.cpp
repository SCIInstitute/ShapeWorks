...
// LdsAnalyze
///////////////////////////////////////////////////////////////////////////////
void LdsAnalyzeCommand::buildParser()
{
  const std::string prog = "ldsanalyze";
  const std::string desc = "Analyze a particle system through LDS Model";
  parser.prog(prog).description(desc);
...
bool LdsAnalyzeCommand::execute(const optparse::Values &options, SharedCommandData &sharedData)
{
  const std::string& projectFile(static_cast<std::string>(options.get("name")));
