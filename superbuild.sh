...
if [[ -z $VTK_DIR ]]; then
  ...
  if [[ $BUILD_CLEAN = 1 ]]; then rm -rf build; fi
  mkdir -p build && cd build
  cmake -DCMAKE_INSTALL_PREFIX=${INSTALL_DIR} -DBUILD_SHARED_LIBS:BOOL=ON -DBUILD_TESTING:BOOL=OFF ${QT_CONFIG} -DVTK_Group_Qt:BOOL=${HAVE_QT} -DVTK_QT_VERSION=5 -Wno-dev ${VTK_DIR}
  make -j${NUM_PROCS} install || exit 1
fi
...
cmake -DCMAKE_INSTALL_PREFIX=${INSTALL_DIR} -DITK_DIR=${INSTALL_DIR} -DVXL_DIR=${INSTALL_DIR} -DVTK_DIR=${INSTALL_DIR} -DBuild_Post:BOOL=${BUILD_POST} -DBuild_View2:BOOL=${HAVE_QT} ${OPENMP_FLAG} ${QT_CONFIG} -Wno-dev -Wno-deprecated ${SRC}
make -j${NUM_PROCS} install || exit 1
