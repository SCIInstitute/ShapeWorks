...
build_vtk()
{
  ...
  if [[ $OSTYPE == msys* ]]; then
      # NOTE: Windows still builds VTK static (no Python wrapping) — the shared+
      # wrapped VTK dedup is enabled on macOS/Linux only for now. Windows
      # conversion is a follow-up (see windows-vtk-handoff). Keep BUILD_SHARED_LIBS
      # OFF here until the Windows DLL-discovery work is done.
