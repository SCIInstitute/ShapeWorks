...
ShapeWorksView2::ShapeWorksView2(int argc, char** argv)
{
  ...
  this->ui->tabWidget->setStyleSheet(QString(
                                       "QTabWidget::pane { border: 2px solid rgb( 80, 80, 80 ); }"));
#endif

  QSize size =
    Preferences::Instance().getSettings().value("mainwindow/size", QSize(1280, 720)).toSize();
  this->resize(size);
...
void ShapeWorksView2::on_actionExportSurfaceMesh_triggered()
{
  ...
  if (seperateFilesForDomains) {
    for (int i = 0; i < this->numDomains; i++) {
      ...
      if (fi.suffix() == "stl") {
        QString domainFilename = fi.dir().path() + QDir::separator() + base +
...
void ShapeWorksView2::handleGroupTimer()
{
  ...
  if (value >= this->ui->pcaGroupSlider->maximum() ||
      value <= this->ui->pcaGroupSlider->minimum()) {
...
{
  this->ui->regressionLabel->setText(QString::number(this->getRegressionValue(this->ui->
                                                                              regressionSlider->
                                                                              value())));
...
void ShapeWorksView2::handleRegressionTimer()
{
  ...
  if (value >= this->ui->regressionSlider->maximum() ||
      value <= this->ui->regressionSlider->minimum()) {
...
void ShapeWorksView2::updateAnalysisMode()
{
  ...
  if (this->ui->tabWidget->currentWidget() == this->ui->samplesTab) {
    ...
    for (int step = 0; step < this->pregenSteps.size(); step++) {
      ...
      if (pregenSample >= this->ui->sampleSpinBox->minimum() &&
...
void ShapeWorksView2::updateSurfaceSettings()
{
  ...
  float smoothingAmount = (float)this->ui->smoothingSlider->value() /
                          (float)this->ui->smoothingSlider->maximum() * 100.0f;
...
void ShapeWorksView2::updateColorScheme()
{
  ...
  for (int i = 0; i < this->numDomains; i++) {
    ...
    this->surfaceActors[i]->GetProperty()->SetDiffuseColor(
      m_ColorSchemes[domainScheme].foreground.r,
...
//  this->regression->SetExplanatory( evars );
  ...
  this->ui->regressionSlider->setValue(
    (middle - this->regressionMin) / this->regressionRange * this->ui->regressionSlider->maximum());
...
bool ShapeWorksView2::readReconstructionParams(char* filename)
{
  ...
  if (elem) {
    ...
    std::cerr << "No distance transforms specified, using old surface reconstruction method!" <<
      std::endl;
    //return EXIT_FAILURE;
  }
  ...
  double normal_angle = 45.0f;
  elem = docHandle.FirstChild("normal_angle").Element();
  if (elem) {
    normal_angle = atof(elem->GetText());
  }

  double decimation_percent = 0.3f;
  elem = docHandle.FirstChild("decimation_percent").Element();
  if (elem) {
    decimation_percent = atof(elem->GetText());
  }
  ...
  surfaceReconstructor->set_normal_angle(normal_angle);
  surfaceReconstructor->set_decimation_percent(decimation_percent);
...
void ShapeWorksView2::computeModeShape()
{
  ...
  for (int step = 0; step < this->pregenSteps.size(); step++) {
    ...
    if (pregenValue >= this->ui->pcaSlider->minimum() &&
        pregenValue <= this->ui->pcaSlider->maximum()) {
      double pcaValue = this->getPcaValue(pregenValue);
      vnl_vector<double> shape;
      if (this->groupsAvailable) {
        shape = this->stats.Group1Mean() + (this->stats.GroupDifference() * groupRatio) +
                (e * (pcaValue * lambda));
      }
      else {
  ...
  if (this->groupsAvailable) {
    this->displayShape(this->stats.Group1Mean() + (this->stats.GroupDifference() * groupRatio) +
                       (e * (pcaSliderValue * lambda)));
  }
  else {
...
void ShapeWorksView2::computeRegressionShape()
{
  ...
  for (int step = 0; step < this->pregenSteps.size(); step++) {
    ...
    if (pregenValue >= this->ui->regressionSlider->minimum() &&
        pregenValue <= this->ui->regressionSlider->maximum()) {
...
{
  // scale value back to range
  return ((double)sliderValue / (double)this->ui->regressionSlider->maximum() *
          this->regressionRange) + this->regressionMin;
}
...
//---------------------------------------------------------------------------
void ShapeWorksView2::trilinearInterpolate(vtkImageData* grad, double x, double y, double z,
