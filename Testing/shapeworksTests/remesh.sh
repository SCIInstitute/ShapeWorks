...
if [[ $? != 0 ]]; then exit -1; fi
shapeworks seed --value 26 readmesh --name $DATA/ellipsoid_0.ply cvddecimate comparemesh --name $DATA/cvdDecimate1.ply
if [[ $? != 0 ]]; then exit -1; fi
shapeworks seed --value 42 readmesh --name $DATA/ellipsoid_01.vtk cvddecimate --percentage 1.0 comparemesh --name $DATA/cvdDecimate2.vtk
