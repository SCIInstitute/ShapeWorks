...
public:
	...
	public:
		...
		EdgeRing(const EdgeRing& ring) { m_node = ring.m_node; m_r = ring.m_r; m_winding = ring.m_winding; }
		void operator = (const EdgeRing& ring) { m_node = ring.m_node; m_r = ring.m_r; m_winding = ring.m_winding; }

		bool empty() { return m_node.empty(); }

		void add(int n, const vec3d& r) { m_node.push_back(n); m_r.push_back(r); }

		void clear() { m_node.clear(); m_r.clear(); }
