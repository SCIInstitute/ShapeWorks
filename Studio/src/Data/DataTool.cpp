...
DataTool::DataTool(Preferences& prefs) : preferences_(prefs)
{
  ...
  connect(ui_->alignment_image_checkbox, &QCheckBox::stateChanged,
          this, &DataTool::alignment_checkbox_changed);
  connect(ui_->alignment_box, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &DataTool::alignment_option_changed);

  connect(ui_->convert_mesh_checkbox, &QCheckBox::stateChanged, this, &DataTool::update_page);
  ...
  connect(ui_->remesh_percent_checkbox, &QCheckBox::toggled, this, &DataTool::update_ui);
  connect(ui_->domain_box, qOverload<int>(&QComboBox::currentIndexChanged),
          this, &DataTool::domain_changed);

  connect(ui_->mesh_smooth, &QCheckBox::stateChanged, this, &DataTool::update_ui);
  connect(ui_->mesh_smooth_method, qOverload<const QString&>(&QComboBox::currentIndexChanged), this,
          &DataTool::update_ui);

  connect(ui_->resample_checkbox, &QCheckBox::stateChanged, this, &DataTool::update_ui);
  connect(ui_->isotropic_checkbox, &QCheckBox::stateChanged, this, &DataTool::update_ui);

  connect(ui_->reflect_checkbox, &QCheckBox::stateChanged, this,
          &DataTool::reflect_checkbox_changed);

  connect(ui_->reflect_column, qOverload<int>(&QComboBox::currentIndexChanged), this,
          &DataTool::reflect_column_changed);

  connect(ui_->reflect_choice, qOverload<int>(&QComboBox::currentIndexChanged), this,
          &DataTool::reflect_choice_changed);

  connect(ui_->reflect_axis, qOverload<int>(&QComboBox::currentIndexChanged), this,
          &DataTool::reflect_axis_changed);

  connect(ui_->remesh_checkbox, &QCheckBox::stateChanged, this, &DataTool::update_ui);
...
//---------------------------------------------------------------------------
void DataTool::on_antialias_checkbox_stateChanged(int state)
{
...
//---------------------------------------------------------------------------
void DataTool::on_blur_checkbox_stateChanged(int state)
{
...
//---------------------------------------------------------------------------
void DataTool::on_autopad_checkbox_stateChanged(int state)
{
...
//---------------------------------------------------------------------------
void DataTool::handle_error(QString msg)
{
...
//---------------------------------------------------------------------------
void DataTool::handle_progress(int val)
{
...
//---------------------------------------------------------------------------
void DataTool::on_restore_defaults_clicked()
{
...
//---------------------------------------------------------------------------
void DataTool::update_reflect_columns()
{
...
//---------------------------------------------------------------------------
void DataTool::update_reflect_choices()
{
  auto project = session_->get_project();
...
//---------------------------------------------------------------------------
void DataTool::set_ui_from_params(GroomParameters params)
{
...
//---------------------------------------------------------------------------
void DataTool::on_run_groom_button_clicked()
{
  if (this->groom_is_running_) {
    shutdown_threads();
  ...
  connect(worker, &ShapeworksWorker::finished, this, &DataTool::handle_thread_complete);
  connect(this->groom_.data(), &QGroom::progress, this, &DataTool::handle_progress);
  connect(worker, &ShapeworksWorker::error_message, this, &DataTool::handle_error);
  connect(worker, &ShapeworksWorker::message, this, &DataTool::message);
...
//---------------------------------------------------------------------------
void DataTool::handle_thread_complete()
{
  emit progress(95);
...
//---------------------------------------------------------------------------
void DataTool::on_skip_button_clicked()
{
...
//---------------------------------------------------------------------------
void DataTool::alignment_checkbox_changed(int state)
{
...
//---------------------------------------------------------------------------
void DataTool::alignment_option_changed(int index)
{
  ui_->alignment_box->setCurrentIndex(index);
}

//---------------------------------------------------------------------------
void DataTool::reflect_checkbox_changed(int state)
{
...
//---------------------------------------------------------------------------
void DataTool::reflect_column_changed(int index)
{
...
//---------------------------------------------------------------------------
void DataTool::reflect_choice_changed(int index)
{
  ui_->reflect_choice->setCurrentIndex(index);
}

//---------------------------------------------------------------------------
void DataTool::reflect_axis_changed(int index)
{
  ui_->reflect_axis->setCurrentIndex(index);
}

//---------------------------------------------------------------------------
void DataTool::shutdown_threads()
{
