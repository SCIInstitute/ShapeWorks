...
const std::string DeepSSMTool::MODE_ALL_SAMPLES_C("all samples");
const std::string DeepSSMTool::MODE_MEAN_C("mean");
const std::string DeepSSMTool::MODE_PCA_C("pca");
const std::string DeepSSMTool::MODE_SINGLE_SAMPLE_C("single sample");
const std::string DeepSSMTool::MODE_REGRESSION_C("regression");
...
DeepSSMTool::DeepSSMTool(Preferences& prefs) : preferences_(prefs)
{
  ...
  connect(this->ui_->group_animate_checkbox, &QCheckBox::stateChanged, this,
          &DeepSSMTool::handle_group_animate_state_changed);
  connect(&this->group_animate_timer_, &QTimer::timeout, this, &DeepSSMTool::handle_group_timer);

  connect(this->ui_->group_box, qOverload<const QString&>(&QComboBox::currentIndexChanged),
          this, &DeepSSMTool::update_group_values);

  connect(this->ui_->group_left, qOverload<const QString&>(&QComboBox::currentIndexChanged),
          this, &DeepSSMTool::group_changed);
...
//  this->ui_->evaluation_widget->hide();
  ...
  for (auto button : {this->ui_->distance_transfom_radio_button,
    ...
    connect(button, &QRadioButton::clicked,
            this, &DeepSSMTool::reconstruction_method_changed);
  }
...
//---------------------------------------------------------------------------
std::string DeepSSMTool::get_analysis_mode()
{
  if (this->ui_->tabWidget->currentWidget() == this->ui_->samples_tab) {
    if (this->ui_->allSamplesRadio->isChecked()) { return DeepSSMTool::MODE_ALL_SAMPLES_C; }
    if (this->ui_->singleSamplesRadio->isChecked()) { return DeepSSMTool::MODE_SINGLE_SAMPLE_C; }
  }

  if (this->ui_->tabWidget->currentWidget() == this->ui_->mean_tab) {
    return DeepSSMTool::MODE_MEAN_C;
  }
  if (this->ui_->tabWidget->currentWidget() == this->ui_->pca_tab) {
    return DeepSSMTool::MODE_PCA_C;
  }
  if (this->ui_->tabWidget->currentWidget() == this->ui_->regression_tab) {
    return DeepSSMTool::MODE_REGRESSION_C;
  }
  return "";
}

//---------------------------------------------------------------------------
bool DeepSSMTool::get_group_difference_mode()
{
  return this->ui_->difference_button->isChecked();
}

//---------------------------------------------------------------------------
std::vector<Shape::Point> DeepSSMTool::get_group_difference_vectors()
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::on_linear_radio_toggled(bool b)
{
  if (b) {
...
//---------------------------------------------------------------------------
void DeepSSMTool::handle_reconstruction_complete()
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::on_reconstructionButton_clicked()
{
  this->store_settings();
  ...
  connect(worker, &ShapeworksWorker::error_message, this, &DeepSSMTool::error);
  connect(worker, &ShapeworksWorker::warning_message, this, &DeepSSMTool::warning);
...
//---------------------------------------------------------------------------
int DeepSSMTool::getPCAMode()
{
  return this->ui_->pcaModeSpinBox->value() - 1;
}

//---------------------------------------------------------------------------
double DeepSSMTool::get_group_value()
{
...
//---------------------------------------------------------------------------
bool DeepSSMTool::pcaAnimate()
{
  return this->ui_->pcaAnimateCheckBox->isChecked();
}

//---------------------------------------------------------------------------
void DeepSSMTool::setLabels(QString which, QString value)
{
...
//---------------------------------------------------------------------------
int DeepSSMTool::get_sample_number()
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::update_analysis_mode()
{
  this->handle_analysis_options();
}

//---------------------------------------------------------------------------
void DeepSSMTool::compute_mode_shape()
{}

//---------------------------------------------------------------------------
void DeepSSMTool::handle_analysis_options()
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::handle_median()
{
...
//-----------------------------------------------------------------------------
void DeepSSMTool::on_mean_button_clicked()
{
...
//-----------------------------------------------------------------------------
void DeepSSMTool::on_group1_button_clicked()
{
...
//-----------------------------------------------------------------------------
void DeepSSMTool::on_group2_button_clicked()
{
...
//-----------------------------------------------------------------------------
void DeepSSMTool::on_difference_button_clicked()
{
  this->ui_->group_slider->setValue(this->ui_->group_slider->minimum());
  this->ui_->mean_button->setChecked(false);
  this->ui_->group1_button->setChecked(false);
...
//-----------------------------------------------------------------------------
bool DeepSSMTool::compute_stats()
{
...
//-----------------------------------------------------------------------------
StudioParticles DeepSSMTool::get_mean_shape_points()
{
...
//-----------------------------------------------------------------------------
StudioParticles DeepSSMTool::get_shape_points(int mode, double value)
{
...
//---------------------------------------------------------------------------
ShapeHandle DeepSSMTool::get_mode_shape(int mode, double value)
{
  return this->create_shape_from_points(this->get_shape_points(mode, value));
}

//---------------------------------------------------------------------------
ParticleShapeStatistics DeepSSMTool::get_stats()
{
  this->compute_stats();
  return this->stats_;
}
...
//---------------------------------------------------------------------------
bool DeepSSMTool::export_variance_graph(QString filename)
{
  return this->ui_->graph_->grab().save(filename);
}

//---------------------------------------------------------------------------
void DeepSSMTool::on_tabWidget_currentChanged()
{
  this->update_analysis_mode();
}

//---------------------------------------------------------------------------
void DeepSSMTool::on_pcaSlider_valueChanged()
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::on_group_slider_valueChanged()
{
  // this will make the slider handle redraw making the UI appear more responsive
  //QCoreApplication::processEvents();
...
//---------------------------------------------------------------------------
void DeepSSMTool::on_pcaModeSpinBox_valueChanged(int i)
{
  emit pca_update();
}

//---------------------------------------------------------------------------
void DeepSSMTool::handle_pca_animate_state_changed()
{
  if (this->pcaAnimate()) {
...
//---------------------------------------------------------------------------
void DeepSSMTool::handle_pca_timer()
{
  if (!this->pcaAnimate()) {
    return;
  }
...
//---------------------------------------------------------------------------
void DeepSSMTool::handle_group_animate_state_changed()
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::handle_group_timer()
{
...
//---------------------------------------------------------------------------
double DeepSSMTool::get_pca_value()
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::pca_labels_changed(QString value, QString eigen, QString lambda)
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::enable_actions(bool newly_enabled)
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::set_analysis_mode(std::string mode)
{
...
//---------------------------------------------------------------------------
ShapeHandle DeepSSMTool::get_mean_shape()
{
...
//---------------------------------------------------------------------------
ShapeHandle DeepSSMTool::create_shape_from_points(StudioParticles points)
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::set_feature_map(const std::string& feature_map)
{
  this->feature_map_ = feature_map;
}

//---------------------------------------------------------------------------
void DeepSSMTool::update_group_boxes()
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::update_group_values()
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::group_changed()
{
  this->stats_ready_ = false;
  this->compute_stats();
}
...
//---------------------------------------------------------------------------
bool DeepSSMTool::groups_active()
{
  std::string group_set = this->ui_->group_box->currentText().toStdString();
...
//---------------------------------------------------------------------------
void DeepSSMTool::on_view_open_button_toggled()
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::on_surface_open_button_toggled()
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::on_metrics_open_button_toggled()
{
...
//---------------------------------------------------------------------------
bool DeepSSMTool::is_group_active(int shape_index)
{
  std::string group_set = this->ui_->group_box->currentText().toStdString();
  std::string left_group = this->ui_->group_left->currentText().toStdString();
  std::string right_group = this->ui_->group_right->currentText().toStdString();
...
//---------------------------------------------------------------------------
void DeepSSMTool::initialize_mesh_warper()
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::reconstruction_method_changed()
{
...
//---------------------------------------------------------------------------
StudioParticles DeepSSMTool::convert_from_combined(const vnl_vector<double>& points)
{
...
//---------------------------------------------------------------------------
void DeepSSMTool::compute_reconstructed_domain_transforms()
{
