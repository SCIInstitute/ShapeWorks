...
void AnalysisTool::handle_analysis_options() {
  if (ui_->tabWidget->currentWidget() == ui_->samples_tab) {
    ...
    ui_->rppcaAnimateCheckBox->setEnabled(true);
    ui_->rppcaModeSpinBox->setEnabled(true);
...
StudioParticles AnalysisTool::get_rppca_shape_points(int mode, double value)
{
  ...
  if (mode + 2 > rppca_job_->RPPCAEigenvalues().size()) {
    mode = rppca_job_->RPPCAEigenvalues().size() - 2;
  }

  // unsigned int m = this->rppca_job_->RPPCAEigenvectors().cols() - (mode + 1);
  unsigned int m = mode;

  Eigen::VectorXd e = rppca_job_->RPPCAEigenvectors().col(m);
  ...
  for (int i = rppca_job_->RPPCAEigenvalues().size() - 1; i > 0; i--) {
    vals.push_back(rppca_job_->RPPCAEigenvalues()[i]);
  }
  double sum = std::accumulate(vals.begin(), vals.end(), 0.0);
  double cumulation = 0;
  for (size_t i = 0; i < mode + 1; ++i) {
    cumulation += rppca_job_->RPPCAExpVar()[i];
  }
  if (sum > 0) {
  ...
  temp_shape_ = rppca_job_->RPPCAMean() + (e * (value * lambda));
...
void AnalysisTool::handle_rppca_timer()
{
  ...
  if (value >= ui_->rppcaSlider->maximum() || value <= ui_->rppcaSlider->minimum()) {
    rppca_animate_direction_ = !rppca_animate_direction_;
  }

  ui_->rppcaSlider->setValue(value);
}
...
void AnalysisTool::set_analysis_mode(std::string mode) {
  if (mode == "all samples" || mode == "single sample") {
    ...
    ui_->tabWidget->setCurrentWidget(ui_->rppca_tab);
