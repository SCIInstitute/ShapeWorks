...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    # Copy libraries from anaconda
    conda_libs="libgfortran libmetis libbtf libcamd libccolamd libcsparse libcxsparse libklu libldl librbio libspqr libsuitesparseconfig libumfpack libcolamd libamd libopenblas libcholmod libboost_system libboost_iostreams libboost_filesystem libbz2 liblzma liblz4 libtbb libHalf libpython libzstd"
    for clib in $conda_libs; do
