...
parser.add_argument("--use_case", help="Specify which use case to run", choices=["ellipsoid", "ellipsoid_mesh", "ellipsoid_fd", "ellipsoid_cut", "lumps", "left_atrium", "femur", "femur_mesh", "femur_cut", "deep_ssm"])
...
parser.add_argument("--skip_grooming", help="Skip the grooming steps and start with already prepped data", action="store_true")
parser.add_argument("--groom_images", help = "Apply grooming steps to both the segmentations and raw images.", action="store_true")
parser.add_argument("--use_single_scale", help="Use single scale optimization. (Default is multi scale.)", action="store_true")
