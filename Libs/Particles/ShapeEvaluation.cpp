...
namespace shapeworks {
    ...
    double ShapeEvaluation<VDimension>::ComputeCompactness(const ParticleSystem &particleSystem, const int nModes, const std::string &saveScreePlotTo) {
    ...
    template <unsigned int VDimension>
    double ShapeEvaluation<VDimension>::ComputeGeneralization(const ParticleSystem &particleSystem, const int nModes, const std::string &saveTo) {
        const int N = particleSystem.N();
        const int D = particleSystem.D();
    ...
    template <unsigned int VDimension>
    double ShapeEvaluation<VDimension>::ComputeSpecificity(const ParticleSystem &particleSystem, const int nModes, const std::string &saveTo) {
        ...
        for (int modeNumber = 0; modeNumber < nModes; modeNumber++) {
            ...
            for (int i = 0; i < nSamples; i++) {
                ...
                for (int j = 0; j < nTrain; j++) {
                    Eigen::Map<const RowMajorMatrix> ptsDistance_vec_reshaped(ptsDistance_vec.col(j).data(), numParticles, VDimension);
    ...
    template class ShapeEvaluation<3>;
}
