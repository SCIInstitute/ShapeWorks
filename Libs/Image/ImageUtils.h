...
public:
  ...
  static LogicalRegion boundingBox(std::vector<std::string> &filenames, Image::PixelType isoValue = 1.0);

  /// calculate bounding box incrementally for shapework images using the region of data <= the given isoValue
  static LogicalRegion boundingBox(std::vector<Image> &images, Image::PixelType isoValue = 1.0);
