 ...
 public:
  ...
  virtual void BeforeIteration() {
    ...
    if (m_RunStatus == 1)  // initialization
    {
      ...
      if (m_GlobalSigma >=
          0.0)  // make sure that we update the global sigma at the beginning (in the constructor, it is -1)
      {
  ...
  inline double ComputeModifiedCotangent(double rij) const {
    if (rij > m_GlobalSigma) return 0.0;
  ...
  void SetRunStatus(int i) { m_RunStatus = i; }

  int GetRunStatus() const { return m_RunStatus; }
