...
class VolumeDomain : public ParticleRegionDomain {
 public:
  ...
  inline PointType GetValidLocationNear(PointType p) const override {
    ...
    return image_->GetZeroCrossingPoint();
  }
  ...
  vnl_vector_fixed<float, DIMENSION> SampleGradientAtPoint_ct(const PointType &point, int idx) const;
 ...
 inline PixelType SampleAtPoint(const PointType &point) const 
 {
  ...
  void DeleteImages() override {
      image_->DeleteImages();
      ct_image_->DeleteImages();
  }

  void DeletePartialDerivativeImages() override {
      image_->DeletePartialDerivativeImages();
      ct_image_->DeletePartialDerivativeImages();
  }

  void SetVolume(ImageType::Pointer image, ImageType::Pointer ct_image, double narrow_band);

  std::shared_ptr<ImplicitSurfaceDomain<ImageType::PixelType>> GetImageCT() const { return ct_image_; }
  // std::shared_ptr<Mesh> GetImage() const { return image_; }

  void UpdateZeroCrossingPoint() override {}

 private:
  std::shared_ptr<ImplicitSurfaceDomain<ImageType::PixelType>> image_;
  std::shared_ptr<ImplicitSurfaceDomain<ImageType::PixelType>> ct_image_;
