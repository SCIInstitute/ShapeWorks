...
void NonLinearCorrespondenceFunction ::ComputeLatentSpaceGradientUpdates() {
  ...
  m_points_mean->clear();
  m_points_mean->set_size(full_num_dims, 1);
  
  // std::cout << "LSU - 1" << std::endl;
  ...
  // std::cout << "all latent space updates calucluated -- 1" << std::endl;
}
...
NonLinearCorrespondenceFunction::VectorType NonLinearCorrespondenceFunction ::Evaluate(unsigned int idx, unsigned int d,
                                                                                       ...
                                                                                       double& maxdt,
                                                                                       double& energy) const {
  // std::cout << "Inside Evaluate - 0" << std::endl;
  ...
  energy = tmp(0, 0);
  // std::cout << "m_PointsUpdateNonLinear has shape " << m_PointsUpdateNonLinear->rows() << " X " << m_PointsUpdateNonLinear->cols() << std::endl;
  for (unsigned int i = 0; i < VDimension; i++) {
    gradE[i] = m_PointsUpdateNonLinear->get(k + i, d / DomainsPerShape);
  }
  // std::cout << "Inside Evaluate - 1" << std::endl;
