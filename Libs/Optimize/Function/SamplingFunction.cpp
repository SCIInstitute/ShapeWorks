...
SamplingFunction::TGradientNumericType SamplingFunction::AngleCoefficient(
  ...
  TGradientNumericType cosine =
      dot_product(p_i_normal, p_j_normal) / (p_i_normal.magnitude() * p_j_normal.magnitude() + 1.0e-6);
...
void SamplingFunction::ComputeAngularWeights(
  ...
  for (unsigned int i = 0; i < neighborhood.size(); i++) {
    weights[i] =
        this->AngleCoefficient(posnormal, domain->SampleNormalAtPoint(neighborhood[i].Point, neighborhood[i].Index));
    if (weights[i] < 1.0e-5) {
      weights[i] = 0.0;
    }
  }
