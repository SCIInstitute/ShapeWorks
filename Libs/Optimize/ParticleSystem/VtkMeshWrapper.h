...
private:
  ...
  // total number of entries will be max_cache_entries_ *
  ...
  mutable FixedSizeCache geo_cache_;
  const GeoEntry& GetGeodesicsBetweenTriangles(int f_a, int f_b, bool need_grad) const;
  ...
  mutable Eigen::VectorXd geo_lq_distances_[3];
  mutable bool geo_lq_have_grad_;
  mutable Eigen::MatrixXd geo_lq_grads_[3];

  Eigen::MatrixXi adjacency_;
  void GetKRing(int src_face, int k, std::unordered_set<int>& out) const;
};
}