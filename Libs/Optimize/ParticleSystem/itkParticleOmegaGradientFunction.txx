...
::BeforeEvaluate( unsigned int idx, unsigned int d, const ParticleSystemType* system )
{
    ...
    // Shadow particle plane constraints
    planePts.clear();
    spherePts.clear();
    CToP.clear();
    int numSpheres = 0;
    ...
    {
        double D = double(planeConsts[pidx].GetPlaneNormal().dot(x_eigen-planeConsts[pidx].GetPlanePoint()));
        itk::Point<double, VDimension> planept;
        for ( unsigned int i = 0; i < VDimension; i++ )
            planept[i] = x[i] - (planeConsts[pidx].GetPlaneNormal()[i] * D);
    ...
    m_CurrentNeighborhood = system->FindNeighborhoodPoints(pos, m_CurrentWeights, neighborhood_radius, "omega::be", d);
...
//            else
    ...
    while ( err != 0 )
    {
        ...
        m_CurrentNeighborhood = system->FindNeighborhoodPoints( pos, m_CurrentWeights, neighborhood_radius, "omega::be2", d );
...
//                else
    ...
    if ( m_CurrentSigma > this->GetMaximumNeighborhoodRadius() )
    {
        ...
        m_CurrentNeighborhood = system->FindNeighborhoodPoints( pos, m_CurrentWeights,
                                                                neighborhood_radius, "omega::be3", d );
