...
bool OptimizeParameterFile::set_optimization_parameters(TiXmlHandle* docHandle, Optimize* optimize)
{
  ...
  elem = docHandle->FirstChild("adaptive_splitting").Element(); // Adaptive splitting temporary
  if (elem) { optimize->SetAdaptiveSplitting((bool) atoi(elem->GetText())); }
...
bool OptimizeParameterFile::read_cutting_ffcs(TiXmlHandle* docHandle, Optimize* optimize)
{
  ...
  elem = docHandle->FirstChild("num_ffcs_per_input").Element();
  if (elem) {
    inputsBuffer.str(elem->GetText());
    double val;

    while (inputsBuffer >> val) {
      ffcs_per_input.push_back(val);
    }
    inputsBuffer.clear();
    inputsBuffer.str("");
