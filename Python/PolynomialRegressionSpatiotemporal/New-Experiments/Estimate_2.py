...
def estimate_mean(shape_matrix, t, fn_mse, fn_r2):
    ...
    exp_var = pca_ob.explained_variance_ratio_
    
    betas_for_low_dim = estimate_parameters_from_pca_embeddings(shape_matrix_transformed, t, fn_mse, fn_r2, exp_var)
    # --> r X (degree+1)
...
def estimate_parameters_from_pca_embeddings(shape_matrix, t, fn_mse, fn_r2, exp_var):
    ...
    # Estimate alpha from MultiTask
    temp_reg = MultiTaskElasticNetCV()
    temp_reg.fit(X, shape_matrix)
    
    alpha_tuned = temp_reg.alpha_
    print(f'Tuned Alpha from Multi Task is {alpha_tuned}')
    betas = np.zeros((dM, degree+1))
    # compute polynomial for each mode
    for r in range(shape_matrix.shape[1]):
        cur_alpha = alpha_tuned/exp_var[r]
        lassoreg = ElasticNet(alpha=cur_alpha)
