...
class Gaussian_Sampler(Sampler):
	def fit(self, embedded_matrix):
		Utils.message("Fitting Gaussian distribution...")
		self.embedded_matrix = embedded_matrix
...
class Mixture_Sampler(Sampler):
	def fit(self, embedded_matrix, mixture_num):
		Utils.message("Fitting Gaussian mixture model...")
		self.embedded_matrix = embedded_matrix
		if mixture_num == 0:
		...
		Utils.message("Gaussian mixture model converged: " + str(self.GMM.converged_))
	...
	def selectClusterNum(self):
		...
		Utils.message("Using " + str(mixture_num) + " components.")
		return mixture_num
	def sample(self):
